...
{
    return update_crc32(0xffffffff, buf, len);
}

uint32_t file_crc32(string file_name)
{
    ...
    if (is)
    {
        ...
        do
        {
            ...
            if (len > 0)
            {
                crc = update_crc32(crc, buffer, len);
                read_count += len;
            }
        } while (len > 0);

        if (read_count == 0 || read_count != length) {
            cout << "read byte "s << read_count << " file lenght "s << length << " error crc for file "s << file_name << endl;
        }
        is.close();
        return crc;
    }
